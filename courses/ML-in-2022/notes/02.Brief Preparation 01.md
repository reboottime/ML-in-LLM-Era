# Preparation Course Notes Part I
...
- Step 2: Define the Loss from Training Data
  ...
  - The output value of the `loss` function represents how good a set of values is. The smaller the `L` value is, the better the parameter sets.
    ![.](./screenshots/L-value.jpg)
...
- Step 3: Optimization: how to find the optimize direction, from 1 -> n
  - if the function only has one parameter
  - then the function has two parameters -> more

- what is `Training` & and Prediction

  ![.](./screenshots/what-is-traning.jpg)
  - we use existing data to train model
  - then ask model to gives prediction result for unseen data
- linear models
